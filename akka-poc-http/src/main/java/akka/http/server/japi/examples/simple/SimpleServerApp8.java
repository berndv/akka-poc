...
public class SimpleServerApp8 extends HttpApp
{
   ...
   public Route createRoute()
   {
      ...
      return route(
         ...
         path("add").route(handleWith(addHandler, x, y)), path("subtract").route(handleWith(x, y, subtractHandler)),
         path("divide")
            .route(handleWith(x, y, (ctx, x, y) -> ctx.complete(String.format("%d / %d = %d", x, y, x / y)))),
         // matches paths like this: /multiply/{x}/{y}
         path("multiply", xSegment, ySegment).route(
         // bind handler by reflection
            handleWith(xSegment, ySegment, SimpleServerApp8::multiply)), path("multiply-methodref", xSegment, ySegment)
            .route(
            // bind handler by reflection
